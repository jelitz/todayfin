...
/**
 * 헤더 아래 얇은 가로 바 — 전 지표를 우→좌로 무한 스크롤(증권방송 시세 티커 느낌).
...
export default function TickerBar({ summary, onSelect }: TickerBarProps): JSX.Element | null {
  ...
  const renderItem = (indicator: (typeof items)[number], key: string) => {
    ...
    const changeText = isFlows ? formatChangeAbs(indicator.change_abs, indicator.unit) : formatPct(indicator.change_pct)
    ...
    return (
      <button key={key} type="button" className="ticker-item" onClick={() => onSelect(indicator.id)}>
        ...
        <span className="ticker-item-value">{indicator.latest === null ? '—' : formatValue(indicator.latest, indicator.unit)}</span>
