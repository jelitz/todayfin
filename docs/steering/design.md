...
- **홈 = 단일 컴팩트 테이블**(2026-08-08, `docs/specs/home-table-view/`): 카드 그리드를 폐기하고 지표당 1행(이름·현재값·등락·스파크라인·기준일·상세 앵커)의 테이블로 전환. 섹션·소그룹은 테이블 내 그룹 헤더 행. max-width **1280px** 유지. 모바일(≤640px)은 추세·기준일 열 숨김 + 상세 아이콘만. 구 3/2/1열 카드 그리드는 알상무 영상 카드에만 남아 있다.
...
- **티커 바**(신규 재도입): GNB 바로 아래, `--surface` 배경, 전 지표 요약을 우→좌 무한 스크롤(OS 모션 설정과 무관하게 항상 재생). hover 정지·클릭 시 상세 모달. 홈(`#/`)·상세(`#/i/{id}`)에서만 노출하고 소개·알상무 페이지에서는 숨긴다.
- 카드(알상무 영상 등 잔존 카드): `--surface` 배경 + 1px `--hairline` + 8px 라운드. 지표 카드(`IndicatorCard`)는 2026-08-08 테이블 전환으로 삭제됨
- **섹션 내 소그룹**: 한 섹션에 성격이 다른 지표가 섞이면(예: 거시·통화의 환율·국채) `--ink-muted` 소제목으로 소그룹을 나눠 모아 보여준다(`types.ts`의 `SECTIONS[].subsections`) — 테이블 전환 후에는 소그룹 헤더 행으로 표현. 섹션은 2026-08-08 "변동성·리스크"(VKOSPI) 추가로 5개. 같은 날 global-indicators로 지표 21개: "시장 가격·추세"는 국내(코스피·코스닥·삼성전자·SK하이닉스)/해외(나스닥·S&P 500·다우존스·니케이 225) 소그룹, 거시·통화의 환율 소그룹은 "환율·달러인덱스"(원/달러·달러/엔·유로/달러·달러인덱스), 원자재는 WTI·금 선물
...
- 수급 상세 집계: 일별/주간/**누적** 3모드. 누적 = 시계열 시작 0 기준 주체별 누적 순매수, 기본 외국인 단독 + 20영업일(4주) 평활선(원본은 50% 투명 가늘게 — `FLOWS_SUBJECT_COLORS_FADED`), 범례 행 우측에 누적·오늘·직전일 요약, 크로스헤어 툴팁에는 평활선 제외
