...
- **섹션 내 소그룹**: 한 섹션에 성격이 다른 지표가 섞이면(예: 거시·통화의 환율·국채) `--ink-muted` 소제목으로 소그룹을 나눠 모아 보여준다(`types.ts`의 `SECTIONS[].subsections`) — 테이블 전환 후에는 소그룹 헤더 행으로 표현. 섹션은 2026-08-08 "변동성·리스크"(VKOSPI) 추가로 5개
