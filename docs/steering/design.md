...
:root {
  ...
  --canvas: #ffffff;
  --surface-soft: #fafafa;
  --surface-dark: #171717;   /* 역색 표면 — 헤더 등 한정된 곳에만(전면 다크모드 아님) */
  /* 텍스트 */
  --ink: #000000;
  --ink-charcoal: #525252;
  --ink-body: #737373;
  --ink-muted: #a3a3a3;
  --on-dark: #ffffff;        /* surface-dark 위 본문 텍스트 */
  --on-dark-mute: rgba(255, 255, 255, 0.7); /* surface-dark 위 보조 텍스트 */
  /* 보더 */
  --hairline: #e5e5e5;
...
1. **UI 크롬은 흑백·회색만.** 색은 데이터 잉크(캔들, 수급 라인, MA 라인, 등락 표시)에만 허용 — Ollama의 "의미 있는 곳에만 색" 철학의 금융 확장.
2. **그림자·그라디언트·장식 금지.** 구분은 1px 헤어라인과 여백으로.
3. **라이트 모드 온리** (원문 원칙). 페이지 전체를 다크로 전환하지 않는다. 단, 원문에 문서화된 `surface-dark`/`on-dark` 역색 표면 패턴은 **헤더 한 곳**에 한정해 사용한다(2026-08-03, 본문과의 시각적 구분 요청 반영) — 유채색이 아니라 흑백 스케일 내 톤이므로 원칙 1과 상충하지 않음.
...
- 수급: 개인/외국인/기관 3주체를 각각 다른 색의 라인으로(회색/파랑/주황 — 외국인이 알상무 기준 핵심 계열이라 가장 눈에 띄는 파랑 배정). 일별/주간집계 토글
