...
- **섹션 내 소그룹**: 한 섹션에 성격이 다른 지표가 섞이면(예: 거시·통화의 환율·국채) `--ink-muted` 소제목(`--fs-body-sm`, 600)으로 소그룹을 나눠 모아 보여준다. 2026-08-03 사용자 피드백으로 "거시·통화"에 환율/미국 국채/한국 국채 3개 소그룹 도입(`types.ts`의 `SECTIONS[].subsections`) — GNB 탭·앵커는 상위 섹션 단위 그대로 유지, 소그룹은 홈 화면 시각적 정리 목적
