...
## 배포 후 사용자 리포트로 발견·수정한 버그 (2026-08-03)

1. **카드 스파크라인이 안 보임**: `tokens.css` 재작성 시 `--ink-charcoal`(#525252, ink와 ink-body 사이 중간 톤) 토큰을 실수로 빠뜨림. `IndicatorCard.css`의 `.indicator-card-spark polyline { stroke: var(--ink-charcoal); }`와 `Detail.css`의 `.detail-ma-option { color: var(--ink-charcoal); }`가 정의되지 않은 변수를 참조하게 되어, `stroke`(상속 속성)가 계산값 무효화로 초기값(`none`)까지 떨어져 스파크라인 선 자체가 렌더링되지 않았음(카드 레이아웃은 정상이라 육안 diff 없이는 놓치기 쉬운 유형). 타입체크·테스트·빌드 전부 이 문제를 못 잡음(CSS 커스텀 프로퍼티 미정의는 컴파일 에러가 아니라 조용히 무효화되기 때문) — 실제 화면을 봐야 드러나는 버그(Stage 3 z-index, Stage 4 base 경로 버그와 동일 계열). `tokens.css` 라이트/다크 양쪽에 `--ink-charcoal` 복원(라이트 #525252 원값 유지, 다크 #c2c2c2 신규 추가)으로 해결.
2. **티커 바가 안 움직이는 것처럼 보임**: 실제 버그가 아니라 최초 설계의 `prefers-reduced-motion: reduce` 접근성 분기가 사용자 브라우저 환경에서 활성화되어 있어 의도대로 정적 표시되고 있었음. 사용자가 "OS 설정과 무관하게 항상 애니메이션"으로 정책을 뒤집기로 결정 → `TickerBar.css`의 `@media (prefers-reduced-motion: reduce)` 블록 제거(`requirements.md` R3 갱신, 접근성보다 항상-동작을 우선하는 명시적 트레이드오프로 기록).
