...
## 사용자 피드백 반영 4차 (2026-08-03, KRX 전환 후 리뷰)

1. **다크모드 line 차트 색상 버그**: line 타입 지표(환율·국채·WTI)의 차트 선이 `CHART_COLOR_LINE = "#000000"` 고정이라 다크모드 배경(`#141414`)에서 거의 안 보이던 버그를 사용자가 리포트. up/down처럼 "등락 의미"를 담은 색이 아니라 순수 UI색이므로 카드 스파크라인과 동일하게 테마를 따르도록 수정 — `getChartSurfaceTheme(theme)`에 `line` 필드 추가(라이트 `#525252`/다크 `#c2c2c2`), `CHART_COLOR_LINE` 상수 제거.
2. **국채 국가별 소그룹핑**: "거시·통화" 섹션에 환율·미국 국채·한국 국채가 뒤섞여 있던 것을 소제목으로 분리. `types.ts`의 `SECTIONS`에 `subsections` 지원을 추가(`HomeSection.ids` 또는 `HomeSection.subsections` 중 하나만 사용), `Home.tsx`는 `IndicatorGrid` 헬퍼로 렌더링 중복 제거. GNB 탭·앵커·`useActiveSection`은 상위 섹션 단위 그대로라 영향 없음.
3. **line 타입 MA 제거**: 2차 피드백(2026-08-03)에서 "일부 지표만 MA가 있으면 혼란스럽다"는 이유로 flows를 제외한 전 지표(ohlcv+line)에 20/60/120일 MA를 통일했었는데, 이번엔 반대로 "주식이 아닌 것들"(환율·국채·WTI)에서 MA를 빼 달라는 요청. 단일 선 차트에서는 MA 오버레이가 정보 밀도만 높인다는 취지로 판단해 `Detail.tsx`의 MA 체크박스 UI·`maPeriods` 계산 조건을 `record.type === 'ohlcv'`로 좁힘 — line 타입은 MA 컨트롤 자체가 사라짐. `requirements.md` R1 MA 컬럼을 "없음"으로 되돌리고 정책 변경 이력을 기록.
4. **홈 소개 문구 축소**: "이 사이트는 유튜브 채널 알상무가 강조해 온 '매일 지표를 확인하는 투자 습관'에서 출발했습니다." 첫 문장 삭제 요청 — 3차 피드백에서 넣었던 기획 의도 3문장 중 첫 문장만 제거, 나머지 두 문장(코멘트 대신 스스로 판단 기준을 세운다는 취지)은 유지.
