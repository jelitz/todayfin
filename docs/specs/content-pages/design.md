...
2. **미디어 수집 파이프라인** — YouTube Data API v3로 최신 영상을 주기적으로 가져와 `data/youtube.json`으로 배포하는, 기존 주가 파이프라인과 완전히 분리된 경로.
...
두 번의 API 호출로 구성된다: `channels.list`(part=snippet,contentDetails)로 채널명과 업로드 플레이리스트 ID를 얻고, `playlistItems.list`(part=snippet,contentDetails)로 최신 15개를 가져온다. 각 1유닛씩 총 2유닛.

업로드 플레이리스트 ID는 채널 ID의 `UC` → `UU` 치환으로도 구할 수 있지만(널리 쓰이는 관행), 공식 보장이 아니라 `channels.list`를 호출한다 — 할당량이 제약이 아닌 상황에서 굳이 비공식 규칙에 의존할 이유가 없고, 채널명도 함께 얻는다.

게시일은 `snippet.publishedAt`(플레이리스트 추가 시각)이 아니라 `contentDetails.videoPublishedAt`(실제 공개 시각)을 쓴다. 비공개 →공개 전환 시 둘이 어긋난다.
...
썸네일이 없는 항목은 `thumbnail_url: None`으로 두고, 프론트에서 플레이스홀더를 표시한다. 해상도는 maxres → standard → high → medium → default 순으로 가장 큰 것을 고른다.

### `pipeline/collect_media.py`

`collect.py`의 스테이징→원자적 교체 패턴을 재사용하되, 훨씬 단순하다(지표 루프·검증·summary 생성 없음):

1. API fetch (재시도 2회, `collect.py`와 동일한 백오프)
2. 최소 검증: `videos`가 비어 있지 않고 각 항목에 `video_id`/`title`/`watch_url`이 있는지
3. `.staging/youtube.json`에 쓴 뒤 `os.replace`로 교체
4. 실패 시 기존 파일 유지하고 **exit 0** — 영상이 안 올라오거나 API가 일시적으로 죽는 것은 정상 범주라 워크플로우를 실패시키지 않는다
...
| `sources/youtube_api.py` | pytest — playlistItems 응답 파싱, 정렬, 썸네일 선택, 비공개·삭제 영상 제외 |
| `collect_media.py` | pytest — fetch 실패 시 기존 파일 보존 |
| 통합 | dev 서버 + claude-in-chrome으로 GNB 탭 전환·티커 노출/숨김·알상무 카드 렌더 확인 |

파싱 테스트는 API 응답 구조를 코드 내 리터럴로 구성해 네트워크 의존을 없앤다.

**소스 변경 이력**: 최초 설계는 유튜브 RSS 피드였으나 GitHub Actions 러너 IP 차단으로 사용 불가해 공식 API로 전환했다 — 진단 과정은 `implemented.md` 참조.
