...
실행: uv run --python 3.12 --with pytest --with requests pytest pipeline/tests/test_collect_media.py
"""
...
def test_collect_writes_json(tmp_path):
    with patch("collect_media.youtube_api.fetch", return_value=_SAMPLE):
...
def test_collect_preserves_existing_file_on_fetch_failure(tmp_path):
    ...
    # time.sleep 목킹 — 실제 재시도 백오프를 그대로 기다리면 테스트가 느려진다
    with (
        patch("collect_media.youtube_api.fetch", side_effect=RuntimeError("네트워크 오류")),
        patch("collect_media.time.sleep"),
    ):
        code = collect_media.collect(str(tmp_path))

    # 실패해도 exit 0 — 영상 미업로드·일시적 API 장애는 정상 범주(requirements.md R5)
    assert code == 0
    with open(tmp_path / "youtube.json", encoding="utf-8") as f:
...
def test_collect_rejects_empty_video_list(tmp_path):
    ...
    with patch("collect_media.youtube_api.fetch", return_value=empty):
        code = collect_media.collect(str(tmp_path))
...
def test_collect_rejects_video_missing_required_field(tmp_path):
    ...
    with patch("collect_media.youtube_api.fetch", return_value=broken):
        code = collect_media.collect(str(tmp_path))
