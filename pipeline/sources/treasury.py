"""미 재무부 Daily Treasury Par Yield Curve Rates — 10년물.
...
def fetch(indicator_id: str, start: date, end: date) -> pd.DataFrame:
    """반환: date, value (%). indicator_id는 'ust10y' 고정."""
    frames = []
    ...
    col10 = next((c for c in raw.columns if "10 Yr" in c), None)
    if col10 is None:
        raise ValueError(f"treasury fetch: '10 Yr' 컬럼 없음 (columns={raw.columns.tolist()})")

    raw["date"] = pd.to_datetime(raw["Date"], format="%m/%d/%Y").dt.strftime("%Y-%m-%d")
    out = raw[["date", col10]].rename(columns={col10: "value"}).dropna()
